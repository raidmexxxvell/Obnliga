...
- Уточнить формат для `league/form` - ответ (Берём streaks (серии результатов) — они понятнее для фанатов. Пример: В-В-Н-П-В).
- Решить, нужен ли общий endpoint для нескольких сезонов (история). - пока не нужен.
- Мониторинг worker → "Добавляем BullBoard в dev-среду для просмотра очередей. Sentry — только для критических ошибок (чтобы не засорять логи)".
- Patch-диффы → "После MVP: сначала проверим стабильность, потом сэкономим трафик через jsondiffpatch".
