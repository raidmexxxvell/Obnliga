...
- `frontend/` — публичный Telegram WebApp: сплэш, новости, профиль, капитанский портал составов, WebSocket-клиент.
...
- **Состояние.** На фронте пока микс: публичная часть опирается на локальные `useState`/`useEffect`, но roadmap `docs/state.md` описывает фасад со стором; админ-панель уже использует Zustand c TTL, версионностью и мультистадией авторизации (admin → judge → assistant → lineup). Серверный state — через Prisma транзакции + кэш-версии в Redis.
