...
##  Среднесрочные цели (обновление 2025-10-05)
- Спроектировать публичные агрегированные эндпоинты и WebSocket-топики (по видам данных). — Статус: 🟨 — Приоритет: 🔴 — Документ: `specs/001-project-analysis/public-aggregates-plan.md`.
- Реализовать воркер, который слушает админские события и обновляет Redis. — Статус: 🟨 — Приоритет: 🔴 — План: in-process BullMQ worker (`stats-aggregation`) внутри Fastify вместо платного Render background worker.
- Перевести админские формы на событие `stats.updated`. — Статус: 🟨 — Приоритет: 🔵 — матч-редактор уже публикует `match:{id}:stats`, остаётся подключить остальные вкладки.
...
Цель: SPA для Telegram WebApp с typed store façade (Zustand/adapter).
 ...
 Система управления составами: полнофункциональная реализация с валидацией, мобильной адаптивностью и синхронизацией UX между админ-панелью и капитанским порталом. — Статус: ✅ — Приоритет: 🔴
    Визуализация плей-офф сетки и выбор формата `PLAYOFF_BRACKET` (админка + портал). — Статус: 🟨 — Приоритет: 🔵 (backend `/api/bracket` реализован как временный stub, портал ещё не подключён)
